...
export default function LobbyScreen() {
  ...
  if (currentRoom && currentRoom.status === "lobby") {
    ...
    return (
      <section className="min-h-screen flex items-center justify-center p-3 md:p-4 pb-20 md:pb-4">
  ...
  return (
    <section className="min-h-screen flex items-start md:items-center justify-center p-3 md:p-4 pb-24 md:pb-4">
      <div className="ui-shell w-full max-w-md md:max-w-3xl">
        <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-5">
          <h1 className="text-4xl md:text-6xl leading-none font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-sky-200 via-amber-300 to-sky-400">
            DEAD SOULS
          </h1>
          <p className="mt-2 text-slate-300/90 tracking-[0.18em] uppercase text-[11px] md:text-sm">Mini App Lobby</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 max-w-3xl mx-auto">
          <motion.article initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} className="panel p-4 md:p-6 min-h-[220px] md:min-h-[240px] flex flex-col">
            ...
            <button onClick={createRoom} className="btn btn-primary w-full mt-3 md:mt-4">
              Создать
            </button>
          </motion.article>
          ...
          <motion.article initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} className="panel p-4 md:p-6 min-h-[220px] md:min-h-[240px] flex flex-col">
            ...
            <div className="mt-4 flex flex-col sm:flex-row gap-2">
              ...
              />
              <button onClick={() => joinRoom(roomInput)} disabled={roomInput.length < 4} className="btn btn-secondary px-4 sm:min-w-[96px]">
                GO
              </button>
            </div>
          </motion.article>
