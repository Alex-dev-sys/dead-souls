...
import { Coins, Eye, Trophy, Skull, Clock, Zap, AlertTriangle, User } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import CityMap from "./CityMap";

const ITEMS_INFO = [
    { id: 'cloak', name: 'Плащ', icon: '🧥', cost: 150, desc: '-20% риска (пасс.)' },
    { id: 'dagger', name: 'Нож', icon: '🔪', cost: 200, desc: '+15% цена (пасс.)' },
    { id: 'bribe', name: 'Взятка', icon: '💰', cost: 100, desc: '-30% розыск (мгновенно)' }
];
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-black/90">
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="bg-card border border-white/10 p-8 rounded-2xl text-center space-y-6 max-w-md w-full">
                    {currentRoom.winner === me.nickname ?
                        <Trophy className="w-24 h-24 text-yellow-400 mx-auto animate-bounce" /> :
                        <Skull className="w-24 h-24 text-gray-500 mx-auto" />}
                    ...
                    {currentRoom.winner && <p className="text-xl text-gray-400">Победитель: <span className="text-white font-bold">{currentRoom.winner}</span></p>}

                    <div className="space-y-2 pt-4 border-t border-white/10">
                        {[...currentRoom.players].sort((a, b) => b.money - a.money).map((p, i) => (
                    ...
                    <button onClick={() => location.reload()} className="w-full py-4 bg-primary text-black font-bold rounded-xl hover:opacity-90">В ЛОББИ</button>
                </motion.div>
            </div>
        );
    }
                        ...
                        <motion.div initial={{ y: 50 }} animate={{ y: 0 }} className="bg-card border border-rose-500/50 p-6 rounded-2xl w-full max-w-md shadow-2xl shadow-rose-900/20">
                            ...
                            <div className="space-y-3">
                                {encounterData.opts.map((opt) => (
                                    <button key={opt.id} onClick={() => resolveEncounter(opt.id)}
                                        className={cn("w-full py-4 rounded-xl font-bold text-white transition-transform active:scale-95", opt.color)}>
                                        {opt.text}
                                    </button>
                                ))}
                            </div>
            ...
            <header className="grid grid-cols-3 md:grid-cols-6 gap-2 bg-card/60 backdrop-blur border border-white/5 p-3 rounded-xl">
                ...
                <div className="col-span-2 flex gap-1 justify-end">
                    {me.activeBuffs.map(b => (
                        <span key={b} className="text-[10px] bg-purple-500/20 text-purple-300 px-2 py-1 rounded border border-purple-500/30 uppercase">{b}</span>
                    ))}
                    {me.items.map(i => (
                        <span key={i} className="text-lg" title={i}>{ITEMS_INFO.find(x => x.id === i)?.icon}</span>
                    ))}
                </div>
            </header>

            {/* Turn Banner */}
            <div className={cn("text-center py-2 rounded-lg font-bold text-sm transition-colors",
                isMyTurn ? "bg-emerald-900/30 text-emerald-400 border border-emerald-500/30" : "bg-black/40 text-gray-600")}>
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-12 gap-2 min-h-0 overflow-hidden">
                ...
                <div className="md:col-span-8 flex flex-col gap-2 min-h-0">
                    ...
                    <div className="flex gap-2">
                        <button onClick={() => setTab('map')} className={cn("flex-1 py-3 rounded-lg font-medium text-sm transition-all", tab === 'map' ? "bg-purple-600 text-white" : "bg-white/5 hover:bg-white/10")}>🗺️ КАРТА ГОРОДА</button>
                        <button onClick={() => setTab('market')} className={cn("flex-1 py-3 rounded-lg font-medium text-sm transition-all", tab === 'market' ? "bg-amber-600 text-white" : "bg-white/5 hover:bg-white/10")}>🛒 ЧЕРНЫЙ РЫНОК</button>
                    </div>

                    <div className="flex-1 bg-black/20 rounded-xl border border-white/5 overflow-hidden relative">
                        {tab === 'map' && <div className="absolute inset-0 flex items-center justify-center"><CityMap /></div>}

                        {tab === 'market' && (
                            <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-3 overflow-y-auto h-full">
                                {ITEMS_INFO.map(item => (
                                    <button key={item.id} onClick={() => isMyTurn && buyItem(item.id)} disabled={!isMyTurn || me.money < item.cost}
                                ...
                                <div className="col-span-full border-t border-white/10 pt-4 mt-2">
                                    ...
                                    <button onClick={() => isMyTurn && activateAbility()} disabled={!isMyTurn || me.abilityCooldown > 0}
                                        ...
                                        <div className="text-left">
                                            ...
                                            <div className="text-xs text-purple-400">
                                                {me.abilityCooldown > 0 ? `Перезарядка ${me.abilityCooldown} ход.` : "ГОТОВО К ИСПОЛЬЗОВАНИЮ"}
                                            </div>
                                        </div>
                                    </button>
                                </div>
                ...
                <div className="md:col-span-4 flex flex-col gap-2 min-h-0">
                    ...
                    <div className="bg-card/50 border border-white/5 rounded-xl p-3 max-h-[30%] overflow-y-auto">
                        <div className="space-y-1">
                            {currentRoom.players.map(p => (
                                <div key={p.socketId} className={cn("flex justify-between items-center text-xs p-2 rounded",
                    ...
                    <div className="flex-1 bg-black/40 rounded-xl border border-white/5 p-3 overflow-y-auto flex flex-col-reverse custom-scrollbar text-xs space-y-reverse space-y-1">
                        {gameLogs.map(l => (
                            <div key={l.id} className={cn("p-2 rounded border-l-2",
                                l.type === 'success' ? "border-emerald-500 bg-emerald-900/20" :
                                    l.type === 'danger' ? "border-rose-500 bg-rose-900/20" :
                    ...
                    {/* Inventory (Small) */}
                    <div className="h-[20%] bg-card/50 border border-white/5 rounded-xl p-2 overflow-y-auto">
                        <h4 className="text-[10px] uppercase text-gray-500 mb-1">Души ({me.inventory.length})</h4>
                        <div className="flex flex-wrap gap-1">
                            {me.inventory.map((s, i) => (
                                <button key={s.id} onClick={() => isMyTurn && sellSoul(i)} disabled={!isMyTurn}
